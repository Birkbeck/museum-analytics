dataCollectionAnalysisUI <- function(id) {
  fluidPage(
    div(
      ...
      h3(id="eventsPerCollectionTitle", "Recorded events by groups of objects according to subject matter"),
      p(style=explanation_para, "In a few cases, the same group of objects may be associated with a sequence of events. Although we may not have detailed information on the constituent objects within the group, we may have recorded information about where that group of objects goes, for instance when it has been repeatedly transferred across institutions."),
      p(style=explanation_para, "The number of sequential events that we recorded per group of objects for each museum categorised by subject matter is shown in the visualisation below. We have used a ‘box and whiskers’ visualisation that indicates the overall distribution of a set of values. The vertical line in the centre is the median value. The box contains 50% of all the values and the whiskers show values that are not outliers. The dots represent outliers. For a detailed explanation see ‘About the Data’."),
      plotlyOutput(NS(id, "eventsPerCollection"), width="80%", height="1000px"),
      tags$a(href = "#top", "⬆ Back to Top"),

      hr(),

      h3(id="collectionGranularityTitle", "Recorded quantities of objects"),
      p(style=explanation_para, "In some cases, we were able to establish approximately how many objects were involved in an event. For example, one object had been sold, one hundred objects had been loaned, one thousand objects had been transferred. In other cases, we could only record quantity as it was expressed as a proportion of the total holdings; as all, most, or some of the collection, or a few objects."),
      p(style=explanation_para, "The table below shows the information recorded about the quantity of objects involved in an event. About 30% of groups of objects were given a numeric size; around 70% were assessed in relative terms."),
      plotlyOutput(NS(id, "collectionGranularity"), width="80%", height="1000px"),
      tags$a(href = "#top", "⬆ Back to Top"),

      h3(id="collectionGranularityHeatmapTitle", "Recorded quantities of objects by subject matter"),
      p(style=explanation_para, "As with the number of events recorded, the information on the size of groups of objects varied according to subject matter. Numerical values are rarely given to groups of objects from museums of the arts, of building, and rural industry. These types of objects are almost always described in relative terms: all, most, or some of the collection."),
      p(style=explanation_para, "The numbers in the visualisation below are the number of groups of objects per museum (the mean). For example, on average we had data on 1.9 individual objects and 1.1 groups of between 2-10 objects for war and conflict museums."),
