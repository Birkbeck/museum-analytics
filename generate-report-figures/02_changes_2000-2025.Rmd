...
write_xlsx(
  ...
  "../data/report_data_01_08_2025/changes_2000-2025.xlsx"
)
```
...
figure_3 <- ggplot(
  ...
  geom_col() +
  geom_text(
    data=changes_by_governance |> filter(change > 0, governance_broad != "unknown governance"),
    aes(label=paste0(change_pc, "%\n(+", change, ")")),
    hjust=1
  ) +
  geom_text(
    data=changes_by_governance |> filter(change < 0, governance_broad != "unknown governance"),
...
ggsave("figures/fig03.svg", figure_3, width=output_width, height=output_height, units=output_units, bg="white")
figure_3
```
...
figure_7 <- ggplot(
  ...
  geom_col() +
  geom_text(
    data=changes_by_size |> filter(change > 0, size != "unknown size"),
    aes(label=paste0(change_pc, "%\n(+", change, ")")),
    hjust=1
  ) +
  geom_text(
...
figure_11 <- ggplot(
  ...
  geom_col() +
  geom_text(
    data=changes_by_subject |> filter(change_pc >= 0),
    aes(label=paste0(change_pc, "% (+", change, ")")),
    size=3,
    hjust=0
  ) +
  geom_text(
...
figure_4 <- ggplot(
  ...
  labs(
    ...
    y = "Governance",
    x = "Number of museum openings/closures" 
  ) +
  standard_bars_theme +
  theme(
...
figure_6 <- ggplot(
  ...
  geom_text(aes(label=governance_broad), size=3, colour="black") +
  coord_fixed() +
  ...
  labs(
    ...
    y="Openings per 100 existing museums"
  ) +
  standard_bars_theme
...
figure_9 <- ggplot(
  ...
  geom_point(alpha=0.7) +
  geom_abline(colour="grey") +
  geom_text(aes(label=size), size=3, colour="black") +
  coord_fixed() +
...
figure_10 <- ggplot(
  changes_by_size_and_governance_with_scale |> filter(change != 0),
...
changes_by_region_and_governance_with_scale <- changes_by_region_and_governance_main |>
  ...
  mutate(z_change=sign(change) * abs(z_change))
figure_15 <- ggplot(
  changes_by_region_and_governance_with_scale |> filter(change != 0),
